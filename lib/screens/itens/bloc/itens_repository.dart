...
class ItensRepository {
  final _dataControlEnum = DataControlEnum.itensFetch;
  ...
  Future<List<ItemModel>> fetch({bool forceReload = false}) async {
    _isFetching = true;
    Completer notificationCompleter = Completer();

    notificationBloc.add(
      LoadingNotificationEvent(message: 'Obtendo itens', completer: notificationCompleter),
    );
    ...
    for (List<dynamic> lista in boxData.values) {
      List<ItemModel> page = lista.map((e) => ItemModel.fromJson(jsonDecode(jsonEncode(e)))).toList();
      data.addAll(page);
    }
  ...
  List<ItemModel> pesquisa(String search) {
    _search = search;
    List<ItemModel> pesquisa = List.from(data.where((element) {
  ...
  Future<void> create({required ItemModel registro}) async {
    await ApiRequest.execute(
      dataControlEnum: DataControlEnum.itensCreate,
      registro: FirebaseRegistrosModel(
        newData: registro.toJson(),
        operation: DataControlEnum.itensCreate.method,
      ),
    );
  }

  Future<void> update({required ItemModel registro}) async {
    await ApiRequest.execute(
  ...
  Future<void> remove({required ItemModel registro}) async {
    await ApiRequest.execute(
      dataControlEnum: DataControlEnum.itensDelete,
      registro: FirebaseRegistrosModel(
        id: registro.id,
        newData: registro.toJson(),
        operation: DataControlEnum.itensDelete.method,
      ),
    );
  }
}
