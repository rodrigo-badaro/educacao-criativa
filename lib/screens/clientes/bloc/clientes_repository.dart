...
class ClientesRepository {
  final _dataControlEnum = DataControlEnum.clientesFetch;
  DataControlEnum get dataControlEnum => _dataControlEnum;

  final List<ClienteModel> _data = [];
  List<ClienteModel> get data => _data;
  ...
  Future<List<ClienteModel>> fetch({bool forceReload = false}) async {
    _isFetching = true;
    Completer notificationCompleter = Completer();

    notificationBloc.add(
      LoadingNotificationEvent(message: 'Obtendo clientes', completer: notificationCompleter),
    );
    ...
    for (List<dynamic> lista in boxData.values) {
      List<ClienteModel> page = lista.map((e) => ClienteModel.fromJson(jsonDecode(jsonEncode(e)))).toList();
      data.addAll(page);
    }
  ...
  List<ClienteModel> pesquisa(String search) {
    _search = search;

    List<ClienteModel> pesquisa = List.from(data.where((element) {
      bool nome = element.nome?.toLowerCase().contains(_search.toLowerCase()) ?? false;
      bool sigla = element.sigla?.toLowerCase().contains(_search.toLowerCase()) ?? false;
      bool descricao = element.descricao?.toLowerCase().contains(_search.toLowerCase()) ?? false;

      if (nome || sigla || descricao) return true;
      return false;
    }));
  ...
  Future<void> create({required ClienteModel registro}) async {
    await ApiRequest.execute(
      dataControlEnum: DataControlEnum.clientesCreate,
      registro: FirebaseRegistrosModel(
        newData: registro.toJson(),
        operation: DataControlEnum.clientesCreate.method,
      ),
    );
  }

  Future<void> update({required ClienteModel registro}) async {
    await ApiRequest.execute(
      dataControlEnum: DataControlEnum.clientesUpdate,
      registro: FirebaseRegistrosModel(
        newData: registro.toJson(),
        operation: DataControlEnum.clientesUpdate.method,
      ),
    );
  }

  Future<void> remove({required ClienteModel registro}) async {
    await ApiRequest.execute(
      dataControlEnum: DataControlEnum.clientesDelete,
      registro: FirebaseRegistrosModel(
        id: registro.id,
        newData: registro.toJson(),
        operation: DataControlEnum.clientesDelete.method,
      ),
    );
  }
}
