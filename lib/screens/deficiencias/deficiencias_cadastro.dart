...
Future<void> cadastroDeficienciasPopUp({required BuildContext context, DeficienciaModel? initialValue}) {
  return Modal.right(
    context: context,
    width: 400,
    body: DeficienciasCadastro(initialValue: initialValue),
  );
}

class DeficienciasCadastro extends StatefulWidget {
  final DeficienciaModel? initialValue;

  const DeficienciasCadastro({super.key, this.initialValue});

  @override
  State<DeficienciasCadastro> createState() => _DeficienciasCadastroState();
}

class _DeficienciasCadastroState extends State<DeficienciasCadastro> {
  ...
  void initState() {
    ...
    _nomeController.text = widget.initialValue?.tipo ?? '';
  ...
  void saveData() {
    DeficienciaModel? initialValue = widget.initialValue;

    if (initialValue == null) {
      return deficienciasBloc.add(
        CreateDeficienciasEvent(
          completer: completer,
          button: button,
          registro: DeficienciaModel(
            tipo: _nomeController.text,
