...
Future<void> cadastroFormatoDeNotasPopUp({required BuildContext context, FormatoNotasModel? initialValue}) {
  return Modal.right(
    context: context,
    width: 400,
    body: FormatoDeNotasCadastro(initialValue: initialValue),
  );
}

class FormatoDeNotasCadastro extends StatefulWidget {
  final FormatoNotasModel? initialValue;

  const FormatoDeNotasCadastro({super.key, this.initialValue});

  @override
  State<FormatoDeNotasCadastro> createState() => _FormatoDeNotasCadastroState();
}

class _FormatoDeNotasCadastroState extends State<FormatoDeNotasCadastro> {
  ...
  void saveData() {
    FormatoNotasModel? initialValue = widget.initialValue;

    if (initialValue == null) {
      return formatoNotasBloc.add(
        CreateFormatoNotasEvent(
          completer: completer,
          button: button,
          registro: FormatoNotasModel(
                      ...
                      return Column(
                        ...
                        children: [
                          ListTile(
                            ...
                            title: H1Small(
                              widget.initialValue == null ? 'NOVO FORMATO DE NOTA' : 'EDITAR FORMATO DE NOTA',
                            ),
