...
class TipoDeUnidadesRepository {
  final _dataControlEnum = DataControlEnum.tipoUnidadeFetch;
  DataControlEnum get dataControlEnum => _dataControlEnum;

  final List<TipoDeUnidadeModel> _data = [];
  List<TipoDeUnidadeModel> get data => _data;
  ...
  Future<List<TipoDeUnidadeModel>> fetch({bool forceReload = false}) async {
    _isFetching = true;
    Completer notificationCompleter = Completer();

    notificationBloc.add(
      LoadingNotificationEvent(message: 'Obtendo tipos de unidades', completer: notificationCompleter),
    );
    ...
    for (List<dynamic> lista in boxData.values) {
      List<TipoDeUnidadeModel> page = lista.map((e) => TipoDeUnidadeModel.fromJson(jsonDecode(jsonEncode(e)))).toList();
      data.addAll(page);
    }
  ...
  List<TipoDeUnidadeModel> pesquisa(String search) {
    _search = search;
    List<TipoDeUnidadeModel> pesquisa = List.from(data.where((element) {
  ...
  Future<void> create({required TipoDeUnidadeModel registro}) async {
    await ApiRequest.execute(
      dataControlEnum: DataControlEnum.tipoUnidadeCreate,
      registro: FirebaseRegistrosModel(
        newData: registro.toJson(),
        operation: DataControlEnum.tipoUnidadeCreate.method,
      ),
    );
  }

  Future<void> update({required TipoDeUnidadeModel registro}) async {
    await ApiRequest.execute(
      dataControlEnum: DataControlEnum.tipoUnidadeUpdate,
      registro: FirebaseRegistrosModel(
        newData: registro.toJson(),
        operation: DataControlEnum.tipoUnidadeUpdate.method,
      ),
    );
  }

  Future<void> remove({required TipoDeUnidadeModel registro}) async {
    await ApiRequest.execute(
      dataControlEnum: DataControlEnum.tipoUnidadeDelete,
      registro: FirebaseRegistrosModel(
        id: registro.id,
        newData: registro.toJson(),
        operation: DataControlEnum.tipoUnidadeDelete.method,
      ),
    );
  }
}
